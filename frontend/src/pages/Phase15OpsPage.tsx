...
export function Phase15OpsPage(): JSX.Element {
  ...
  return (
    <section className="min-h-[calc(100dvh-64px)] bg-navy-50">
      ...
      <div className="grid gap-4 p-4 sm:p-6">
        <Card className="p-4">
          ...
          >
            <label className="grid gap-1 text-sm font-semibold text-navy-600">Dashboard logo URL
              <input className="h-10 rounded-lg border border-navy-200 px-3" value={dashboardLogo} onChange={(e) => setDashboardLogo(e.target.value)} placeholder={String(org?.dashboard_logo_url ?? "")} />
            </label>
            <label className="grid gap-1 text-sm font-semibold text-navy-600">Primary color
              <input className="h-10 rounded-lg border border-navy-200 px-3" type="color" value={dashboardColor} onChange={(e) => setDashboardColor(e.target.value)} />
            </label>
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-navy-600"><input type="checkbox" checked={helpWidgetEnabled} onChange={(e) => setHelpWidgetEnabled(e.target.checked)} /> Enable help/support widget</label>
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-navy-600"><input type="checkbox" checked={statusPagePublic} onChange={(e) => setStatusPagePublic(e.target.checked)} /> Public status page enabled</label>
        ...
        <Card className="p-4">
          ...
          >
            <input className="h-10 rounded-lg border border-navy-200 px-3" value={incidentTitle} onChange={(e) => setIncidentTitle(e.target.value)} placeholder="Incident title" />
            <select className="h-10 rounded-lg border border-navy-200 px-3" value={incidentImpact} onChange={(e) => setIncidentImpact(e.target.value)}>
              <option value="minor">minor</option>
              <option value="major">major</option>
              <option value="critical">critical</option>
            </select>
            <input className="h-10 rounded-lg border border-navy-200 px-3 md:col-span-2" value={incidentComponents} onChange={(e) => setIncidentComponents(e.target.value)} placeholder="Components (comma separated)" />
            <textarea className="min-h-20 rounded-lg border border-navy-200 px-3 py-2 md:col-span-2" value={incidentBody} onChange={(e) => setIncidentBody(e.target.value)} placeholder="Incident details" />
          ...
          <div className="mt-4 grid gap-2">
            {incidents.map((row) => (
              <div key={row.id} className="rounded-lg border border-navy-100 bg-white p-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="text-sm font-black">{row.title} <span className="text-xs font-semibold text-navy-400">({row.status})</span></div>
                  {row.status !== "resolved" ? <Button size="sm" variant="secondary" onClick={() => resolveIncident.mutate(row)}>Resolve</Button> : null}
                </div>
                <div className="mt-1 text-xs text-navy-500">{row.body}</div>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-4">
          ...
          >
            <div className="grid gap-3 md:grid-cols-2">
              <input className="h-10 rounded-lg border border-navy-200 px-3" value={blogSlug} onChange={(e) => setBlogSlug(e.target.value)} placeholder="slug (example: launch-checklist)" />
              <input className="h-10 rounded-lg border border-navy-200 px-3" value={blogTitle} onChange={(e) => setBlogTitle(e.target.value)} placeholder="Title" />
            </div>
            <input className="h-10 rounded-lg border border-navy-200 px-3" value={blogExcerpt} onChange={(e) => setBlogExcerpt(e.target.value)} placeholder="Excerpt" />
            <input className="h-10 rounded-lg border border-navy-200 px-3" value={blogTags} onChange={(e) => setBlogTags(e.target.value)} placeholder="Tags (comma separated)" />
            <textarea className="min-h-40 rounded-lg border border-navy-200 px-3 py-2 font-mono text-sm" value={blogContent} onChange={(e) => setBlogContent(e.target.value)} placeholder="# Heading\nYour content..." />
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-navy-600"><input type="checkbox" checked={blogPublished} onChange={(e) => setBlogPublished(e.target.checked)} /> Publish immediately</label>
            <Button variant="primary" disabled={saveBlog.isPending}>{saveBlog.isPending ? "Saving..." : "Save post"}</Button>
          </form>
          <div className="mt-4 grid gap-2">
            {posts.map((row) => (
              <div key={row.id} className="rounded-lg border border-navy-100 bg-white p-3 text-sm">
                <div className="font-black">{row.title}</div>
                <div className="text-xs text-navy-400">/{row.slug} · {row.is_published ? "published" : "draft"}</div>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="mt-2 text-xs text-navy-400">Current: {String(domainSettings?.domain ?? "not configured")}</div>
          <div className="mt-1 text-xs text-navy-400">Status: {String(domainSettings?.verification?.status ?? "unconfigured")}</div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <input className="h-10 rounded-lg border border-navy-200 px-3" value={dashboardDomain} onChange={(e) => setDashboardDomain(e.target.value)} placeholder="support.company.com" />
            ...
            <input className="h-10 rounded-lg border border-navy-200 px-3 md:col-span-2" value={dashboardToken} onChange={(e) => setDashboardToken(e.target.value)} placeholder="Paste TXT verification token value to confirm" />
          ...
          {domainSettings?.verification?.dns ? <pre className="mt-3 overflow-x-auto rounded bg-navy-900 p-3 text-xs text-navy-100">{JSON.stringify(domainSettings.verification.dns, null, 2)}</pre> : null}
        </Card>

        <Card className="p-4">
          <h2 className="text-lg font-black">Custom email sender domain (DKIM/SPF)</h2>
          <div className="mt-2 text-xs text-navy-400">Current: {String(senderSettings?.domain ?? "not configured")}</div>
          <div className="mt-1 text-xs text-navy-400">Status: {String(senderSettings?.verification?.status ?? "unconfigured")}</div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <input className="h-10 rounded-lg border border-navy-200 px-3" value={senderDomain} onChange={(e) => setSenderDomain(e.target.value)} placeholder="mail.company.com" />
            ...
            <input className="h-10 rounded-lg border border-navy-200 px-3 md:col-span-2" value={senderDkim} onChange={(e) => setSenderDkim(e.target.value)} placeholder="Paste DKIM TXT record value" />
            <input className="h-10 rounded-lg border border-navy-200 px-3 md:col-span-2" value={senderSpf} onChange={(e) => setSenderSpf(e.target.value)} placeholder="Paste SPF TXT value" />
          ...
          {senderSettings?.verification?.dns ? <pre className="mt-3 overflow-x-auto rounded bg-navy-900 p-3 text-xs text-navy-100">{JSON.stringify(senderSettings.verification.dns, null, 2)}</pre> : null}
        </Card>

        <Card className="p-4">
          <h2 className="text-lg font-black">Onboarding drip</h2>
          <p className="mt-1 text-sm text-navy-500">Sends staged onboarding emails (day 0 / day 2 / day 7) to active team users.</p>
