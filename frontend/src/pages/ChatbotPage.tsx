...
export function ChatbotPage(): JSX.Element {
  ...
  return (
    <div className="flex h-full">
        <aside className="w-72 shrink-0 border-r border-navy-100 dark:border-navy-700 bg-navy-50 p-3 dark:bg-navy-900">
          ...
          <div className="space-y-1">
            {flows.map((f) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded px-2 py-2 text-left text-xs font-bold ${selectedId === f.id ? "bg-purple-100 dark:bg-purple-900/30" : "hover:bg-navy-100 dark:hover:bg-navy-800"}`}
              >
                <span className="truncate">{f.name}</span>
                <span className={`text-[10px] uppercase ${f.status === "active" ? "text-emerald-600" : "text-navy-400"}`}>{f.status}</span>
              </button>
            ))}
            {flows.length === 0 && <div className="px-2 text-xs text-navy-400">No flows yet</div>}
          </div>
        </aside>

        <main className="flex-1 overflow-y-auto p-4">
          {!editing && (
            <div className="grid place-items-center py-20 text-sm text-navy-400">Select or create a flow.</div>
          )}
          {editing && (
            <div className="mx-auto max-w-3xl space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded border border-navy-100 dark:border-navy-700 bg-white px-3 py-2 text-sm font-black dark:bg-navy-900"
                />
                <select
                  value={editing.status}
                  onChange={(e) => setDraft({ ...editing, status: e.target.value })}
                  className="rounded border border-navy-100 dark:border-navy-700 bg-white px-2 py-2 text-xs font-bold dark:bg-navy-900"
                >
                ...
                <button
                  ...
                  className="rounded bg-navy-900 px-3 py-2 text-xs font-bold text-white hover:bg-navy-800 dark:bg-white dark:text-navy-700"
                >
              ...
              <textarea
                ...
                rows={2}
                className="w-full rounded border border-navy-100 dark:border-navy-700 bg-white px-3 py-2 text-xs dark:bg-navy-900"
              />

              <div className="rounded-lg border border-navy-100 dark:border-navy-700 bg-white p-3 dark:bg-navy-900">
                <div className="mb-2 text-xs font-black uppercase text-navy-400">Trigger</div>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <input
                    ...
                    className="rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs"
                  />
                  <input
                    type="number"
              ...
              <div className="rounded-lg border border-navy-100 dark:border-navy-700 bg-white p-3 dark:bg-navy-900">
                <div className="mb-2 flex items-center justify-between">
                  <div className="text-xs font-black uppercase text-navy-400">Nodes ({editing.nodes.length})</div>
                  <div className="flex flex-wrap gap-1">
                    {(["message", "question", "condition", "action", "faq", "handoff"] as const).map((t) => (
                      <button key={t} onClick={() => addNode(t)} className="rounded border border-navy-100 dark:border-navy-700 px-2 py-1 text-[11px] font-bold hover:bg-navy-100">
                        + {t}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  {editing.nodes.map((node, idx) => (
                    <div key={node.id} className="rounded border border-navy-100 dark:border-navy-700 p-2">
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="flex-1 rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs font-mono"
                        />
                      </div>
                      {(node.type === "message" || node.type === "question" || node.type === "handoff") && (
                        <textarea
                          ...
                          placeholder="Text"
                          rows={2}
                          className="w-full rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs"
                        />
                      )}
                      {node.type === "question" && (
                        <input
                          ...
                          className="mt-1 w-full rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs"
                        />
                      )}
                      {node.type === "condition" && (
              ...
              <div className="rounded-lg border border-navy-100 dark:border-navy-700 bg-white p-3 dark:bg-navy-900">
                <div className="mb-2 text-xs font-black uppercase text-navy-400">Edges</div>
              ...
              {selectedId && analytics && (
                <div className="rounded-lg border border-navy-100 dark:border-navy-700 bg-white p-3 dark:bg-navy-900">
                  <div className="mb-2 flex items-center gap-2 text-xs font-black uppercase text-navy-400"><BarChart3 size={12} /> Analytics</div>
    ...
    <div className="space-y-2">
      {flow.edges.map((e, i) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          <span className="rounded bg-navy-100 px-2 py-1 font-mono">{e.from}</span>
          <span>→</span>
          <span className="rounded bg-navy-100 px-2 py-1 font-mono">{e.to}</span>
      ...
      <div className="grid grid-cols-4 gap-1">
        <select value={from} onChange={(e) => setFrom(e.target.value)} className="rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs">
          <option value="">From</option>
          {flow.nodes.map((n) => <option key={n.id} value={n.id}>{n.id}</option>)}
        </select>
        <select value={to} onChange={(e) => setTo(e.target.value)} className="rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs">
          <option value="">To</option>
          {flow.nodes.map((n) => <option key={n.id} value={n.id}>{n.id}</option>)}
        </select>
        <input value={branch} onChange={(e) => setBranch(e.target.value)} placeholder="branch (true/false)" className="rounded border border-navy-100 dark:border-navy-700 bg-navy-50 px-2 py-1 text-xs" />
        <button
...
function Stat({ label, value }: { label: string; value: number | string }): JSX.Element {
  return (
    <div className="rounded-lg bg-navy-50 p-2 dark:bg-navy-800">
      <div className="text-[10px] font-black uppercase text-navy-400">{label}</div>
