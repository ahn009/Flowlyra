...
export function TicketsPage(): JSX.Element {
  ...
  return (
    <PageShell>
      ...
      />
      {creating && (
        <div className="border-b border-navy-100 dark:border-navy-700 bg-white dark:bg-navy-900 p-4">
          <Card className="p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-2 text-sm font-semibold text-navy-600 sm:col-span-2">
                Subject
              ...
              <label className="grid gap-2 text-sm font-semibold text-navy-600 dark:text-navy-300">
                Priority
                <select className="h-10 rounded-lg border border-navy-100 dark:border-navy-700 bg-white px-3 dark:bg-navy-900 dark:text-navy-100" value={priority} onChange={(event) => setPriority(event.target.value as (typeof priorityOptions)[number])}>
      ...
      <div className="border-b border-navy-100 dark:border-navy-700 bg-white dark:bg-navy-900 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <label className="flex min-w-0 items-center gap-2 rounded-lg border border-navy-100 bg-white px-3 text-sm text-navy-400 focus-within:border-brand-500 focus-within:ring-4 focus-within:ring-brand-500/20 dark:border-navy-700 dark:bg-navy-900 dark:focus-within:ring-brand-500/30 lg:w-72">
      ...
      <div className="p-4 sm:p-6">
        <Card className="overflow-hidden">
          {isLoading ? (
            <div className="p-4 text-sm font-semibold text-navy-400 dark:text-navy-400">Loading tickets...</div>
          ) : filtered.length ? (
            <div className="divide-y divide-border">
              {filtered.map((ticket) => (
                <div key={ticket.id} className="grid gap-2 px-4 py-4 transition hover:bg-navy-50 dark:bg-navy-800/50 md:grid-cols-[26px_90px_minmax(0,1fr)_110px_110px_150px] md:items-center md:gap-3 md:px-5">
                  <input type="checkbox" checked={selectedIds.includes(ticket.id)} onChange={() => toggle(ticket.id)} />
                  <Link to={`/ticket/${ticket.id}`} className="font-semibold text-navy-700 dark:text-navy-100">#{ticket.ticket_number}</Link>
                  <Link to={`/ticket/${ticket.id}`} className="min-w-0 truncate text-sm font-semibold text-navy-700 dark:text-navy-200">{ticket.subject}</Link>
                  <Badge tone={`status-${ticket.status}`}>{ticket.status}</Badge>
                  <Badge tone={`priority-${ticket.priority}`}>{ticket.priority}</Badge>
                  <span className="text-xs font-semibold text-navy-400 md:text-sm">{new Date(ticket.updated_at).toLocaleString()}</span>
                </div>
              ))}
            </div>
          ) : (
  ...
  if (isLoading) {
    return (
      <PageShell>
        ...
        <div className="p-6 text-sm font-semibold text-navy-400 dark:text-navy-400">Loading ticket...</div>
      </PageShell>
    );
  }
    ...
    <PageShell>
      ...
      <div className="grid grid-cols-1 gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-6">
        <Card className="min-h-[420px] p-4 sm:p-6">
          {data?.linked_chat_id && <Link className="mb-4 block rounded-lg border border-brand-200 bg-brand-50 p-3 text-sm font-semibold text-brand-700 transition hover:border-brand-300 hover:bg-brand-100 dark:border-brand-900/60 dark:bg-brand-950/30 dark:text-brand-300" to={`/inbox/chat/${data.linked_chat_id}`}>View original chat</Link>}
          ...
          <div className="mt-4 rounded-lg border border-navy-100 dark:border-navy-700 p-3">
            <div className="text-xs font-black uppercase tracking-wide text-navy-400">Activity timeline</div>
            <div className="mt-2 grid gap-2">
              {(data?.activity ?? []).slice(0, 12).map((item: { id: string; title: string; event_type: string; created_at: string; body?: string }) => (
                <div key={item.id} className="rounded-lg border border-navy-100 dark:border-navy-700 bg-navy-50 p-2 text-xs">
                  <div className="font-bold">{item.title}</div>
                  {item.body ? <div className="text-navy-500">{item.body}</div> : null}
                  <div className="text-navy-400">{item.event_type} · {new Date(item.created_at).toLocaleString()}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="mt-4 grid gap-3">
            {(data?.comments ?? []).map((comment: { id: string; content: string; is_internal: boolean; content_format?: string }) => (
              <div key={comment.id} className={`rounded-lg border p-3 text-sm ${comment.is_internal ? "border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200" : "border-navy-100 dark:border-navy-700 bg-white dark:bg-navy-800 dark:text-navy-200"}`}>
                <div className="mb-1 text-[11px] font-black uppercase text-navy-400">{comment.content_format ?? "plain"}</div>
                {comment.content}
              </div>
            ))}
          </div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <label className="inline-flex items-center gap-2 text-sm font-semibold text-navy-500 dark:text-navy-400">
        ...
        <Card className="p-4">
          ...
          <div className="mt-3 grid gap-3">
            <label className="grid gap-1 text-sm font-semibold text-navy-600 dark:text-navy-300">
              Status
              <select className="h-10 rounded-lg border border-navy-100 dark:border-navy-700 bg-white px-3 dark:bg-navy-900 dark:text-navy-100" value={statusValue} onChange={(event) => setStatusValue(event.target.value)}>
                {statusOptions.map((item) => <option key={item} value={item}>{item}</option>)}
              </select>
            </label>
            <label className="grid gap-1 text-sm font-semibold text-navy-600 dark:text-navy-300">
              Priority
              <select className="h-10 rounded-lg border border-navy-100 dark:border-navy-700 bg-white px-3 dark:bg-navy-900 dark:text-navy-100" value={priorityValue} onChange={(event) => setPriorityValue(event.target.value)}>
                {priorityOptions.map((item) => <option key={item} value={item}>{item}</option>)}
              </select>
            </label>
            <label className="grid gap-1 text-sm font-semibold text-navy-600 dark:text-navy-300">
              Assignee
              <select className="h-10 rounded-lg border border-navy-100 dark:border-navy-700 bg-white px-3 dark:bg-navy-900 dark:text-navy-100" value={assignedUserId} onChange={(event) => setAssignedUserId(event.target.value)}>
                <option value="">Unassigned</option>
                {agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.full_name}</option>)}
              </select>
            </label>
            <div className="rounded-lg border border-navy-100 dark:border-navy-700 p-3 text-sm text-navy-500 dark:text-navy-400">
            ...
            <div className="rounded-lg border border-navy-100 dark:border-navy-700 p-3">
              <div className="text-xs font-black uppercase tracking-wide text-navy-400">Merge tickets</div>
            ...
            <div className="rounded-lg border border-navy-100 dark:border-navy-700 p-3">
              <div className="text-xs font-black uppercase tracking-wide text-navy-400">Split ticket</div>
            ...
            <div className="rounded-lg border border-navy-100 dark:border-navy-700 p-3">
              <div className="text-xs font-black uppercase tracking-wide text-navy-400">Time tracking</div>
