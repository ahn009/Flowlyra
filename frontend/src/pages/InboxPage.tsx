  ...
  return (
    <PageShell>
      ...
      <div className="grid min-h-[calc(100dvh-128px)] grid-cols-1 lg:grid-cols-[minmax(320px,420px)_minmax(0,1fr)]">
        <div className="min-w-0 border-b border-border bg-white lg:border-b-0 lg:border-r">
          <div className="border-b border-border p-3 sm:p-4">
            ...
            <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-3">
              {["All", "Waiting", "Active", "Resolved"].map((label, index) => <button key={label} className={`shrink-0 rounded-lg border px-3 py-1.5 text-sm font-semibold ${index === 0 ? "border-blue-200 bg-blue-50 text-primary" : "border-border text-slate-600 hover:bg-slate-50"}`}>{label}</button>)}
            </div>
            <label className="flex items-center gap-2 rounded-lg border border-border bg-white px-3 text-sm text-slate-500 focus-within:border-blue-300 focus-within:ring-4 focus-within:ring-blue-100">
          ...
          <div className="max-h-[520px] overflow-y-auto lg:max-h-[calc(100dvh-285px)]">
            {rows.length ? rows.map((chat) => <ChatRow key={chat.id} chat={chat} />) : <EmptyState />}
          </div>
        </div>
...
function ChatRow({ chat }: { chat: Chat }): JSX.Element {
  return (
    <Link to={`/chat/${chat.id}`} className="flex min-w-0 items-center gap-3 border-b border-border px-4 py-3 outline-none transition hover:bg-slate-50 focus-visible:bg-blue-50">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 items-center gap-2">
          <div className="truncate font-semibold">{chat.visitor_name || chat.visitor_email || "Website visitor"}</div>
        ...
        <div className="mt-1 flex items-center gap-1 text-xs text-slate-400"><Clock size={12} />{formatTime(chat.updated_at)}</div>
      </div>
...
function ChatStatusBadge({ status }: { status: Chat["status"] }): JSX.Element {
