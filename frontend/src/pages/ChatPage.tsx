        ...
        <div className="flex min-h-[calc(100dvh-64px)] min-w-0 flex-col xl:h-[calc(100dvh-64px)]">
          ...
          <div className="min-h-0 flex-1 overflow-hidden px-3 py-3 sm:px-4 lg:px-6 lg:py-5">
            <div className="mx-auto flex h-full max-w-5xl flex-col overflow-hidden rounded-lg border border-border bg-white shadow-soft dark:bg-slate-900">
              ...
              {preview && (
                <div className="border-b border-blue-100 bg-blue-50 px-5 py-3 text-sm text-blue-950">
...
function MessageRow({ message }: { message: Message }): JSX.Element {
  if (message.is_internal) return <div className="px-2 py-3 text-center text-sm text-amber-800 sm:px-8"><span className="inline-block max-w-full rounded-full border border-amber-200 bg-amber-50 px-3 py-2 font-semibold">Internal note: {message.content}</span></div>;
  ...
  return (
    <div className={`flex items-end gap-2 px-1 py-2 sm:gap-3 sm:px-5 sm:py-3 ${mine ? "justify-start" : "justify-end"}`}>
      ...
      <div className={`max-w-[82%] overflow-hidden break-words rounded-xl px-3 py-2 text-sm leading-6 shadow-sm sm:max-w-[72%] sm:rounded-2xl sm:px-4 sm:py-3 ${mine ? "rounded-bl-md bg-blue-600 text-white" : "rounded-br-md border border-slate-200 bg-white text-slate-900"}`}>
        {mine && <div className="mb-1 text-[11px] font-black uppercase tracking-wide text-blue-100">Support agent</div>}
        {message.content}
        <div className={`mt-1 text-[10px] font-semibold ${mine ? "text-blue-100" : "text-slate-400"}`}>{formatTime(message.created_at)}</div>
      </div>
...
function EmptyConversation(): JSX.Element {
  return <div className="grid min-h-full place-items-center px-4 py-10 text-center text-sm font-semibold text-slate-500">No messages yet.</div>;
}

function Action({ icon, label }: { icon: ReactNode; label: string }): JSX.Element {
  return <button className="inline-flex shrink-0 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50">{icon}{label}</button>;
}
  ...
  return (
    <aside className="border-t border-border bg-white p-3 dark:bg-slate-900 sm:p-5 xl:h-[calc(100dvh-64px)] xl:overflow-auto xl:border-l xl:border-t-0">
      ...
      <div className="mt-4 grid gap-3 text-sm text-slate-600 dark:text-slate-300 md:grid-cols-2 xl:grid-cols-1">
        ...
        <InfoCard title="Internal notes" lines={["Notes are hidden from the widget."]} />
      </div>
    </aside>
  );
}
...
function VisitorPresence({ status }: { status: "online" | "offline" }): JSX.Element {
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-black ${status === "online" ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"}`}><span className={`h-2 w-2 rounded-full ${status === "online" ? "bg-success" : "bg-slate-400"}`} />Visitor {status}</span>;
}
...
function statusTone(status: Chat["status"]): string {
  ...
  if (status === "resolved" || status === "closed") return "bg-slate-100 text-slate-700";
      ...
      <div className="mx-auto max-w-5xl rounded-lg border border-border bg-white p-3 shadow-soft dark:bg-slate-900">
        <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
          <button
            ...
            className={`inline-flex shrink-0 items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black ${noteMode ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
          >
