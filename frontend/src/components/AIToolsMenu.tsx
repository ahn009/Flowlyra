...
export function AIToolsMenu({ value, onApply }: Props): JSX.Element {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute bottom-full left-0 z-30 mb-1 w-56 rounded-xl border border-border bg-white p-2 shadow-xl dark:bg-slate-900">
          {OPS.map(({ op, label }) => (
            <button
              key={op}
              disabled={busy !== null}
              onClick={() => void run(op)}
              className="flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-xs font-bold hover:bg-slate-100 disabled:opacity-50 dark:hover:bg-slate-800"
            >
              <span>{label}</span>
