...
export function CopilotPanel({ chatId, open, onClose, onInsert }: Props): JSX.Element | null {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-border bg-white shadow-2xl dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-border bg-gradient-to-r from-purple-50 to-blue-50 px-4 py-3 dark:from-purple-900/20 dark:to-blue-900/20">
        <div className="flex items-center gap-2">
          <Sparkles size={18} className="text-purple-600" />
          <div className="text-sm font-black">Copilot</div>
        </div>
        <button onClick={onClose} className="rounded p-1 hover:bg-white/50 dark:hover:bg-slate-800">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4 text-sm">
        ...
        {history.map((h, i) => (
          <div key={i} className="space-y-2">
            <div className="rounded-lg bg-slate-100 p-2 text-xs font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-200">
              You: {h.q}
            </div>
            <div className="rounded-lg border border-purple-100 bg-purple-50 p-3 text-xs dark:border-purple-900 dark:bg-purple-900/10">
              <div className="whitespace-pre-wrap text-slate-800 dark:text-slate-100">{h.a.answer}</div>
              {h.a.sources.length > 0 && (
                <div className="mt-2 space-y-1 border-t border-purple-200 pt-2 dark:border-purple-800">
                  <div className="text-[10px] font-black uppercase text-slate-500">Sources</div>
      ...
      <div className="border-t border-border p-3">
        <textarea
          ...
          rows={2}
          className="w-full resize-none rounded-lg border border-border bg-slate-50 p-2 text-sm outline-none focus:bg-white dark:bg-slate-800 dark:focus:bg-slate-800"
          onKeyDown={(e) => {
